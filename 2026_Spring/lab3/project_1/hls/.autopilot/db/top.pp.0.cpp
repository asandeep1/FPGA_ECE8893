...
#pragma HLS inline off
 LOAD_LOOP: for (int i = 0; i < (1 << 16) / 32; i++) {
...
#pragma HLS array_partition variable=vec_b.p complete
 ...
 VITIS_LOOP_326_1: for (int k = 0; k < 16; k++) {
#pragma HLS UNROLL
...
#pragma HLS inline off
 static data_t x_prev_0 = 0;
    ...
    FRONT_32_LOOP: for (int k = 0; k < (1 << 16) / 32; k++) {
#pragma HLS PIPELINE II=1
...
#pragma HLS array_partition variable=total_x0 complete
 ...
 VITIS_LOOP_363_1: for(int i=0; i<16; i++) {
#pragma HLS UNROLL
 total_x0[i] = (data_t)((acc_t)0.875 * (acc_t)va.p[i] + (acc_t)0.125);
        ...
        VITIS_LOOP_372_2: for(int i=0; i<32; i++) {
#pragma HLS UNROLL
...
#pragma HLS inline off
 VITIS_LOOP_395_1: for (int b = 0; b < ((1 << 16) / 256); b++) {
        acc_t sum_abs = 0;
        VITIS_LOOP_397_2: for (int i = 0; i < 256 / 32; i++) {
#pragma HLS PIPELINE II=1
 data_vec_t va = in_a.read();
            data_vec_t vb = in_b.read();
            acc_t s1[32];
#pragma HLS array_partition variable=s1 complete
 VITIS_LOOP_403_3: for(int j=0; j<16; j++) {
#pragma HLS UNROLL
...
#pragma HLS array_partition variable=s2 complete
 VITIS_LOOP_410_4: for(int j=0; j<16; j++) {
#pragma HLS UNROLL
...
#pragma HLS array_partition variable=s3 complete
 ...
 VITIS_LOOP_417_5: for(int j=0; j<8; j++) {
#pragma HLS UNROLL
 s3[j] = s2[2*j] + s2[2*j+1];
            }
            sum_abs += (s3[0] + s3[1] + s3[2] + s3[3] + s3[4] + s3[5] + s3[6] + s3[7]);
        }
        stat_t avg = (stat_t)(sum_abs / (acc_t)256);
...
#pragma HLS inline off
 VITIS_LOOP_431_1: for (int b = 0; b < ((1 << 16) / 256); b++) {
        stat_t st = in_st.read();
        stat_t inv_st = (stat_t)((acc_t)1.0 / (acc_t)st);
        VITIS_LOOP_434_2: for (int i = 0; i < 256 / 32; i++) {
#pragma HLS PIPELINE II=1
 data_vec_t va = in_a.read();
            data_vec_t vb = in_b.read();
            data_vec_t ra, rb;
...
#pragma HLS array_partition variable=rb.p complete
 VITIS_LOOP_441_3: for (int j = 0; j < 16; j++) {
#pragma HLS UNROLL
...
#pragma HLS inline off
 VITIS_LOOP_455_1: for (int k = 0; k < (1 << 16) / 32; k++) {
#pragma HLS PIPELINE II=1
 data_vec_t va = in_a.read();
        data_vec_t vb = in_b.read();
        data_vec_t ra, rb;
#pragma HLS array_partition variable=ra.p complete
#pragma HLS array_partition variable=rb.p complete
 VITIS_LOOP_462_2: for (int i = 0; i < 16; i++) {
#pragma HLS UNROLL
...
#pragma HLS inline off
 STORE_LOOP: for (int i = 0; i < (1 << 16) / 32; i++) {
#pragma HLS PIPELINE II=1
 data_vec_t va = in_a.read();
        data_vec_t vb = in_b.read();

        super_wide_t raw = 0;
        VITIS_LOOP_481_1: for (int k = 0; k < 16; k++) {
#pragma HLS UNROLL
