...
#pragma HLS array_partition variable=vec_b.p complete
 ...
 VITIS_LOOP_30_1: for (int k = 0; k < 16; k++) {
#pragma HLS UNROLL
...
#pragma HLS PIPELINE II=1
 data_vec_t va = in_a.read();
        ...
        wide_dt raw = 0;
        VITIS_LOOP_48_1: for (int k = 0; k < 16; k++) {
#pragma HLS UNROLL
...
#pragma HLS inline off
 static data_t x_prev_0 = 0;
    ...
    FRONT_LOOP: for (int k = 0; k < (1 << 16) / 32; k++) {
#pragma HLS PIPELINE II=1
 data_vec_t va = in_a.read();
        data_vec_t vb = in_b.read();
...
#pragma HLS array_partition variable=tx0 complete
 ...
 VITIS_LOOP_80_1: for(int i=0; i<16; i++) {
#pragma HLS UNROLL
...
#pragma HLS bind_op variable=m_b op=mul impl=dsp latency=3
 tx0[i] = (data_t)(m_a + (acc_t)0.125);
        ...
        VITIS_LOOP_92_2: for(int i=0; i<32; i++) {
#pragma HLS UNROLL
 data_t x0 = tx0[i];
...
#pragma HLS inline off
 VITIS_LOOP_113_1: for (int b = 0; b < ((1 << 16) / 256); b++) {
        acc_t block_sum = 0;
        VITIS_LOOP_115_2: for (int i = 0; i < 256 / 32; i++) {
#pragma HLS PIPELINE II=1
 data_vec_t va = in_a.read();
            data_vec_t vb = in_b.read();
            acc_t lane_sum = 0;
            VITIS_LOOP_120_3: for(int j=0; j<16; j++) {
#pragma HLS UNROLL
 lane_sum += (va.p[j] < 0) ? (acc_t)-va.p[j] : (acc_t)va.p[j];
        ...
        out_stat.write((stat_t)(block_sum / (acc_t)256) + (stat_t)0.5);
    }
}
...
#pragma HLS inline off
 VITIS_LOOP_162_1: for (int k = 0; k < (1 << 16) / 32; k++) {
#pragma HLS PIPELINE II=1
 data_vec_t va = in_a.read(); data_vec_t vb = in_b.read();
        data_vec_t ra, rb;
        VITIS_LOOP_166_2: for (int i = 0; i < 16; i++) {
#pragma HLS UNROLL
