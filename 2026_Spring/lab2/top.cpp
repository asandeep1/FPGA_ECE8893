...
// 2. Memory Reader (Optimized for Burst)
void read_input(const data_t in[NX][NY], hls::stream<data_t>& strm) {
    for (int i = 0; i < NX * NY; i++) {
        #pragma HLS PIPELINE II=1
        // Vitis HLS will widen this access to 512-bit based on top-level pragmas
        strm.write(((data_t*)in)[i]);
    }
}

// 3. The Processing Block (Unchanged logic, optimized for streaming)
void stencil_stage(hls::stream<data_t>& in_strm, hls::stream<data_t>& out_strm) {
    data_t line_buf[2][NY];
    #pragma HLS ARRAY_PARTITION variable=line_buf dim=1 complete
    data_t window[3][3];
...
// 4. Memory Writer (Optimized for Burst)
void write_output(hls::stream<data_t>& strm, data_t out[NX][NY]) {
    for (int i = 0; i < NX * NY; i++) {
        #pragma HLS PIPELINE II=1
        ((data_t*)out)[i] = strm.read();
...
// 5. Top Level with 512-bit Bus Widening
void top_kernel(const data_t A_in[NX][NY], data_t A_out[NX][NY]) {
    // Interface pragmas configured for 512-bit bus width (64 bytes)
    ...
    // This tells the compiler to widen the AXI data bus to 512 bits
    #pragma HLS interface s_axilite port=return
    
    // Explicitly set the bus width to 512 bits
